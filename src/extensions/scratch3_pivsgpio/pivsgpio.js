// "./node_modules/scratch-vm/src/extensions/scratch3_pivsgpio/index.js":
/*!***************************************************************************!*\\
  !*** ./node_modules/scratch-vm/src/extensions/scratch3_pivsgpio/index.js ***!
  \\***************************************************************************/
...
var gpio = window.require(path.join(process.resourcesPath + "/static", 'gpiolib.node'));
/**
...
var Scratch3PiVSGPIOBlocks = /*#__PURE__*/function () {
  _createClass(Scratch3PiVSGPIOBlocks, null, [{
    key: "EXTENSION_NAME",
  ...
  function Scratch3PiVSGPIOBlocks(runtime) {
    _classCallCheck(this, Scratch3PiVSGPIOBlocks);
  ...
  _createClass(Scratch3PiVSGPIOBlocks, [{
    key: "getInfo",
    ...
    key: "when_gpio",
    value: function when_gpio(args) {
      var pin = Cast.toNumber(args.GPIO);
      var val = Cast.toString(args.HILO);
      state = gpio.get(pin, 0, 2); // Get state of pin, set pin as input, make pull up

      if (state == 1) {
        if (val == 'released') return true;else return false;
      } else {
        if (val == 'pressed') return true;else return false;
      }
    } // Get pin state (set pin as input)    

  }, {
    key: "get_gpio",
    value: function get_gpio(args) {
      var pin = Cast.toNumber(args.GPIO);
      var val = Cast.toString(args.HILO);
      state = gpio.get(pin, 0, 2); // Get state of pin, set pin as input, make pull up
    ...
    key: "set_gpio",
    value: function set_gpio(args) {
      if (Cast.toString(args.HILO) == "on") drive = 1;else drive = 0;
      gpio.set(Cast.toNumber(args.GPIO), drive);
    } // Toggle output pin state

  }, {
    key: "toggle_gpio",
    value: function toggle_gpio(args) {
      gpio.toggle(Cast.toNumber(args.GPIO));
    }
  }]);

  return Scratch3PiVSGPIOBlocks;
}();
