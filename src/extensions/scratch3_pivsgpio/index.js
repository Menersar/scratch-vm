...
class Scratch3PiVSGPIOBlocks {
   ...
   */
        ...
        return {
            ...
            blocks: [{
                ...
                text: formatMessage({
                    ...
                    default: 'Sobald Button [GPIO] [HILO] wurde',
    ...
    when_gpio (args) {
        ...
        // const state = gpio.get(pin, 0, 2); // Get state of pin, set pin as input, make pull up
        const state = EditorPreload.gpioGet(pin, 0, 2); // Get state of pin, set pin as input, make pull up

        if (state === 1) {
            if (val === 'released') return true; return false;
        } else if (val === 'pressed') return true; return false;
    }
    ...
    set_gpio (args) {
        let drive = 0;
        if (Cast.toString(args.HILO) === 'on') drive = 1;
    ...
    toggle_gpio (args) {
        ...
        EditorPreload.gpioToggle(Cast.toNumber(args.GPIO));
    }
}
