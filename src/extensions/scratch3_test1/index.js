...
/**
 * Class for the test1 blocks.
 * @constructor
 */
class Scratch3Test1Blocks {
    constructor (runtime) {
        /**
    ...
    /**
     * The key to load & store a target's test1 state.
     * @return {string} The key.
     */
    static get STATE_KEY () {
        return 'Scratch.test1';
    }
     ...
     * @type {Test1State}
     */
    static get DEFAULT_TEST1_STATE () {
        return {
            voiceId: ALTO_ID
        };
    }
     ...
     * @returns {Test1State} the mutable state associated with that target. This will be created if necessary.
     * @private
     */
    _getState (target) {
        let state = target.getCustomState(Scratch3Test1Blocks.STATE_KEY);
        if (!state) {
            state = Clone.simple(Scratch3Test1Blocks.DEFAULT_TEST1_STATE);
            target.setCustomState(Scratch3Test1Blocks.STATE_KEY, state);
        }
        return state;
    }
    ...
    _onTargetCreated (newTarget, sourceTarget) {
        if (sourceTarget) {
            const state = sourceTarget.getCustomState(Scratch3Test1Blocks.STATE_KEY);
            if (state) {
                newTarget.setCustomState(Scratch3Test1Blocks.STATE_KEY, Clone.simple(state));
            }
        }
    }
    ...
    getInfo () {
        ...
        // supported language.
        let defaultTest1 = 'hello';
        if (this.isSupportedLanguage(this.getEditorLanguage())) {
            defaultTest1 = formatMessage({
                id: 'test1.defaultTest1',
                default: 'hello',
...
module.exports = Scratch3Test1Blocks;
