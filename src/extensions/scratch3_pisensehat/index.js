...
class Scratch3PiSenseHatBlocks {
   ...
   */
     ...
     */
        ...
        let fbtest = 0;
        while (1) {
            const fname = `/sys/class/graphics/fb${fbtest.toString()}/name`;
   ...
   */
    ...
    get_temp () {
        if (this.fbfile === '/dev/shm/rpi-sense-emu-screen') {
            ...
            // return EditorPreload.fsSyncOpenReadCloseReturnDataViewNumber('/dev/shm/rpi-sense-emu-pressure', 'r');
            // EditorPreload.fsSyncOpenReadCloseReturnData();
        } return Number(this._imu[4]);
    }
    get_press () {
        if (this.fbfile === '/dev/shm/rpi-sense-emu-screen') {
            const data = new Uint8Array(20);
    ...
    get_oz () {
        ...
        } return Number(this._imu[2]);
    }
    _pixel_remap (pos) {
        const x = pos % 8;
        const y = (pos - x) / 8;
        // eslint-disable-next-line max-len
        if (this._orient === 90) return x * 8 + (7 - y); else if (this._orient === 180) return 63 - (y * 8 + x); else if (this._orient === 270) return (7 - x) * 8 + y; return pos;
    }
    set_pixel (args) {
    ...
    display_symbol (args) {
        ...
        const pix = new Uint8Array(128);
        for (let count = 0; count < 64; count++) {
            let val;
            if (symbol.charAt(count) === '1') val = valf; else val = valb;
    ...
    display_sprite (args, util) {
        ...
        this.smallCtx.drawImage(this.bigCanvas, 0, 0, squareSize, squareSize, 0, 0, 8, 8);
        const matrixImageData = this.smallCtx.getImageData(0, 0, 8, 8);
        this._displayImageData(matrixImageData);
        this.smallCtx.clearRect(0, 0, this.smallCanvas.width, this.smallCanvas.height);
    ...
    _map_orient (pos, orient) {
        if (orient === 0) {
            const x = pos % 8;
            const y = (pos - x) / 8;
            if (x > 5) return 48; return (x * 1 + 1) * 8 - 1 - y * 1;
        } else if (orient === 90) {
            if (pos < 48) return pos; return 48;
        } else if (orient === 180) {
            const x = pos % 8;
            const y = (pos - x) / 8;
            if (x < 2) return 48; return (7 - x * 1) * 8 + y * 1;
        } else if (orient === 270) {
            if (pos > 16) return 63 - pos; return 48;
        }
        return 48;
    }
    _load_letter (lett) {
        const dict = " +-*/!\"#$><0123456789.=)(ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz?,;:|@%[&_']\\\\~";
        const lgr = new Uint8Array(80);
        let inv = 90 - dict.indexOf(lett);
        if (inv > 90) inv = 90;
        const fd = fs.openSync('/usr/lib/scratch3/sense_hat_text.bmp', 'r');
        // eslint-disable-next-line max-len
        for (let count = 0; count < 5; count++) fs.readSync(fd, lgr, count * 16, 16, 3098 + inv * 80 + (64 - count * 16));
        fs.closeSync(fd);
        return lgr;
    }
    _letter (lett, orient, valf, valb) {
        ...
        for (let count = 0; count < 64; count++) {
            ...
            if (map > 7 && map < 48 && lgr[(map - 8) * 2] === 0xFF) val = valf; else val = valb;
            pix[count * 2] = val / 256;
            pix[count * 2 + 1] = val % 256;
        }
        const fd = fs.openSync(this.fbfile, 'r+');
    ...
    _message (message, orient, valf, valb) {
        ...
        fs.writeSync(fd, pix, 0, 128, 0);
        fs.closeSync(fd);
        for (lettInd = 0; lettInd < message.length + 2; lettInd++) {
            const lgrid = this._load_letter(message[lettInd]);
            for (charInd = 0; charInd < 6; charInd++) {
                // scroll the grid
                ...
                for (let row = 0; row < 8; row++) {
                    if (orient === 0) {
                        if (charInd > 5) {
                            pix[row * 16 + 14] = bg0;
                            pix[row * 16 + 15] = bg1;
                        } else if (lgrid[charInd * 16 + (14 - row * 2)] === 0xFF) {
                            pix[row * 16 + 14] = pix0;
                            pix[row * 16 + 15] = pix1;
                        } else {
                            pix[row * 16 + 14] = bg0;
                ...
                // output the buffer
                const fd2 = fs.openSync(this.fbfile, 'r+');
                fs.writeSync(fd2, pix, 0, 128, 0);
                fs.closeSync(fd2);
    ...
    when_tilted (args) {
        ...
        let dir = 0;
        // eslint-disable-next-line max-len
        if (tilt === 'forward') dir = 1; else if (tilt === 'right') dir = 2; else if (tilt === 'backward') dir = 3; else if (tilt === 'left') dir = 4;
        // eslint-disable-next-line max-len
        if (this._orient === 90) dir += 1; else if (this._orient === 180) dir += 2; else if (this._orient === 270) dir += 3;
        if (dir > 4) dir -= 4;
        let xTilt = 0;
        if (x < -15 && x > -90) xTilt = 1;
        if (x > 15 && x < 90) xTilt = -1;
        let yTilt = 0;
        if (y < -15 && y > -90) yTilt = 1;
        if (y > 15 && y < 90) yTilt = -1;
        if (this._xtilt !== xTilt) {
            if (dir === 1 && xTilt === 1 || dir === 3 && xTilt === -1) {
                this._xtilt = xTilt;
                return true;
            }
            if (xTilt === 0) this._xtilt = xTilt;
        }
        if (this._ytilt !== yTilt) {
            if (dir === 2 && yTilt === 1 || dir === 4 && yTilt === -1) {
                this._ytilt = yTilt;
                return true;
            }
            if (yTilt === 0) this._ytilt = yTilt;
        }
        return false;
    }
}
