...
class VirtualMachine extends EventEmitter {
     ...
     */
    // ??? Exact reason for the use of '!!' here? !!!
    ...
    loadProject (input) {
        ...
        const validationPromise = new Promise((resolve, reject) => {
            ...
            validate(input, false, (error, res) => {
                ...
                // !!!!
                // !!! Isn't the following code the same as 'if (error) return reject(error)' reason for changing it? ???
                if (error) {
                    return reject(error);
                }
                resolve(res);
     ...
     */
    // !!! Exact reason for change from 'Sb3' to 'Zip'? ???
    _saveProjectZip () {
    ...
    /**
     * Serialize the project into a map of files without actually zipping the project.
     ...
     */
    // !!! Why here 'require' and not at the top of the file? ???
    async _loadExtensions (extensionIDs, extensionURLs = new Map()) {
        const defaultExtensionURLs = require('./extension-support/sidekick-default-extension-urls');
    ...
    async installTargets (targets, extensions, wholeProject) {
        ...
        return this._loadExtensions(extensions.extensionIDs, extensions.extensionURLs).then(() => {
            ...
            // !!!!
            // !!! Exact meaning of 'wholeProject' here; what exactly is checked here? ???
            if (wholeProject) {
    ...
    // !!!!
    /**
     * Get the raw binary data to use when exporting a costume to the user's local file system.
    ...
    /**
     * Get a base64 string to use when exporting a costume to the user's local file system.
     * @param {Costume} costumeObject scratch-vm costume object
