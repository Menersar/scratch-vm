...
class ScriptTreeGenerator {
    ...
    descendInput (block) {
        ...
        case 'argument_reporter_boolean': {
            ...
            const index = this.script.arguments.lastIndexOf(name);
            if (index === -1) {
                if (name.toLowerCase() === 'is compiled?' || name.toLowerCase() === 'is sidekick?') {
                    return {
                        kind: 'constant',
                        value: true
                    };
    ...
    readTopBlockComment (commentId) {
        ...
        for (const line of text.split('\n')) {
            ...
            // ??? ??? !!! !!!
            // (Source: https://www.w3schools.com/jsref/jsref_regexp_begin.asp)
            // !!! CHANGE ???
            // Search for the pattern 'sidekick' at the end of the string 'line'.
            if (!/^sidekick\b/.test(line)) {
                continue;
            }
            ...
            // !!! ???
            // Only the first 'sidekick' line is parsed.
            break;
        }
    }
