...
class RenderedTarget extends Target {
    ...
    constructor (sprite, runtime) {
        ...
        // !!! 'node-style'?, 'overridden elsewhere'? ???
        // To increase performance, as node-style event emitters have non-zero performance overhead (unlike function calls):
        // Replace some high frequency events with these methods (overridden elsewhere).
     ...
     */
    // Compiler utilizes this function.
    setXY (x, y, force) {
    ...
    setSize (size) {
        ...
        if (this.renderer) {
            ...
            // !!! ???
            // Update size via 'setSize' even without a renderer.
            // (Needed for 'sidekick-change-size-does-not-use-rounded-size.sb3' test.)
            this.size = size;
    ...
    setCostume (index) {
        ...
        // !!! Exact reason for change from '[Infinity, -Infinity, NaN].includes(index)' to 'index === Infinity || index === -Infinity || !index'? ???
