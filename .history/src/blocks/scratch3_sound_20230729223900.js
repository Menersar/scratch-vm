...
class Scratch3SoundBlocks {
    ...
    /** The minimum and maximum values for each sound effect without set limits. */
    static get LARGER_EFFECT_RANGE () {
        return {
            pitch: {min: -1000, max: 1000}, // Limit pitch to prevent scratch-audio throwing errors due to some calculations resulting in infinity.
            pan: {min: -100, max: 100} // 100% left to 100% right, does not add functionality to increase these absolute values.
        };
    }
    ...
    _updateEffect (args, util, change) {
        ...
        // Do not 'yield until the next tick' for every case (so it does not slow down the project for every case).
        // (Just necessary e.g. for the case 'forever: change pitch by 1'.)
