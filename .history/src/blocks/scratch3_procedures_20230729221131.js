class Scratch3ProcedureBlocks {
    ...
    argumentReporterStringNumber (args, util) {
        ...
        if (value === null) {
            // Added to support legacy blocks.
    ...
    argumentReporterBoolean (args, util) {
        ...
        if (value === null) {
            ...
            // Added to support blocks that check if compiled
            if (util.target.runtime.compilerOptions.enabled && lowercaseValue === 'is compiled?') {
                return true;
            }
            // Added to support blocks that check if sidekick.
            if (lowercaseValue === 'is sidekick?') {
                return true;
            }
            // When the parameter is not found in the most recent procedure
            // call, the default is always 0.
