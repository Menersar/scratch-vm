...
const {parseVectorMetadata} = require('../serialization/sidekick-costume-import-export');
...
const loadVector_ = function (costume, runtime, rotationCenter, optVersion) {
    return new Promise(resolve => {
        ...
        // Provide SVG rotation centre information via a dedicated comment.
...
// !!! Example use case in the application? ???
/**
...
/**
 * @param {Asset} asset - a scratch-storage asset.
 * @returns {Promise<HTMLImageElement|ImageBitmap>}
 */
const _persistentReadImage = async asset => {
    // !!! TODO: Maybe find a 'better' solution (perhaps less 'hacky')? !!!
    // When loading a high amount of images at once, image reading may even throw an error on valid images (especially in Chrome).
    // The reading process retries a few times with delays to compensate for it.
    let firstError;
    for (let i = 0; i < 3; i++) {
        try {
            if (typeof createImageBitmap === 'function') {
                ...
                // Running too many 'createImageBitmap' simultaneously some browsers (Chrome D':) may resolve with undefined.
                // Limiting the possible number of possible simultaneous runs should prevent this from happening.
                // !!! 'scratch's broken costume handling'? ???
                // (If it does nonetheless, an error is thrown so it can be retried or it falls back to scratch's broken costume handling).
                if (!imageBitmap) {
...
// Limit the possible number of '_persistentReadImage' via 'AsyncLimiter', as running too many 'createImageBitmap' simultaneously results in browsers breaking.
...
const fetchBitmapCanvas_ = function (costume, runtime, rotationCenter) {
    ...
    return Promise.all([costume.asset, costume.textLayerAsset].map(asset => {
        ...
        .then(([baseImageElement, textImageElement]) => {
            ...
            // So Sidekick/scratch-render receives the information that this canvas will not be reused by the canvas pool.
            // For memory use optimization.
            ...
            return {
                ...
                // !!! Reason for change from 'canvas', 'mergeCanvas' to 'imageOrCanvas'? ???
                image: imageOrCanvas,
...
const loadBitmap_ = function (costume, runtime, _rotationCenter) {
    return fetchBitmapCanvas_(costume, runtime, _rotationCenter)
        ...
        // !!! Reason for change from 'canvas', 'mergeCanvas' to 'image' (and difference here between 'image' and 'imageOrCanvas')? ???
        .then(({image, rotationCenter}) => {
            ...
            if (!rotationCenter) {
                ...
                // ??? Reason for the last 'TODO' statement? ???
                costume.bitmapResolution = 2;
            }
